...
- 후속 증상: 확인 카드의 실행 버튼이 이유 없이 비활성화된다. 현장 점검에서 3100 서버 연결 거부를 확인했고, 코드에는 최초 health 실패 후 재시도 경로가 없었다.
- 결제 조건은 완화하지 않는다. 버튼 옆에 연결/저장 요청 확인 중·확인 실패 이유를 표시하고, 읽기 전용 재확인으로 회복한다. 확인 실패를 Mock 모드라고 표시하지 않는다.
- health 및 저장 요청 GET은 제한 시간 내 실패 상태로 전환한다. 재확인은 create/run을 호출하지 않고, 요청 초안·카드·기존 purchaseId를 유지한다. 404/미확정 거래를 자동 삭제해 우회하지 않는다.
- 영향 범위: 구매 컴포넌트와 집중 브라우저 테스트, 최소 안내/기록. 한 수정 패킷·핵심 흐름 리뷰 1회·기본 lint/build 1회. 계약·결제 백엔드·외부 작업은 제외한다.
