...
1. `.env.local`에 이미 설정된 사용자 PBLC 지갑을 확인한다. 기존 `MONGODB_URI`가 standalone이어도 live runner는 이를 바꾸지 않고, 프로젝트 안의 `.aegis-live-mongo/`에 별도 지속형 replica set을 만들어 실제 결제 시연의 새 감사 기록만 저장한다. 내부 서비스 토큰이 비어 있으면 runner가 실행 중 메모리에서만 임시 생성한다. 개인키는 화면이나 채팅에 붙여넣지 않는다.
   암호화·세션 키가 비어 있으면 먼저 아래를 한 번 실행한다. 기존 값은 바꾸지 않고 새 값도 출력하지 않는다.
