...
function detail(purchaseId: string) {
  return api<PurchaseDetail>(`/purchases/${encodeURIComponent(purchaseId)}`, { cache: "no-store", signal: AbortSignal.timeout(8_000) });
}
...
export function PurchaseRequest() {
  ...
  const refreshReadiness = useCallback(async (): Promise<PendingResolution | null> => {
    if (readinessLock.current) return null;
    readinessLock.current = true;
    if (mounted.current) {
      setPending(null);
    ...
    try {
      const pendingResult = (async () => {
        ...
        return resolvePendingRequest(window.localStorage, detail);
      })();
      const healthResult = api<{ execution_mode?: unknown; token_symbol?: unknown }>("/health", { cache: "no-store", signal: AbortSignal.timeout(8_000) });
      const [pendingSettled, healthSettled] = await Promise.allSettled([pendingResult, healthResult]);
      const resolution = pendingSettled.status === "fulfilled" ? pendingSettled.value : null;
      const health = healthSettled.status === "fulfilled" ? healthSettled.value : null;
      const executionMode = health?.execution_mode === "mock" || health?.execution_mode === "live" ? health.execution_mode : null;
      if (mounted.current) {
        setPending(resolution);
        setPendingIssue(pendingSettled.status === "rejected" ? "브라우저 저장소 또는 저장된 구매를 확인할 수 없습니다. 연결을 다시 확인하세요." : null);
      ...
      return executionMode !== null ? resolution : null;
    } finally {
      readinessLock.current = false;
    }
  }, []);
  ...
  async function confirm(card: Card) {
    ...
    try {
      ...
      if (!purchaseId) {
        ...
        const verified = await refreshReadiness();
        if (verified === null || verified.status === "blocked") throw new Error("새 구매 요청을 다시 확인할 수 없어 실행을 중단했습니다.");
      }
  ...
  const readinessMessage = busy ? "구매 실행 중입니다." : pendingChecking || mode === "checking" ? "연결 및 저장된 구매를 확인 중입니다." : mode === "unavailable" ? "결제 모드를 확인할 수 없습니다. 연결을 다시 확인하세요." : pendingIssue ?? (pending?.status === "blocked" ? "저장된 미완료 구매를 확인하지 못했습니다. 연결을 다시 확인하세요." : null);
  const confirmBlocked = busy || pendingChecking || pending === null || pending.status === "blocked" || mode === "checking" || mode === "unavailable";
  return <section className="requestChatSurface" id="request-chat">
    ...
    <div className="chatThread" aria-live="polite">
      ...
      {cards.length === 0 && readinessMessage && <div className="notice risk" data-testid="purchase-readiness">{readinessMessage} {pending?.purchaseId && <Link href={`/purchases/${encodeURIComponent(pending.purchaseId)}`}>저장된 거래 보기 →</Link>}<button type="button" onClick={() => void refreshReadiness()} disabled={busy || pendingChecking}>연결 다시 확인</button></div>}
      {cards.map((card) => <article className="chatTurn" data-testid="purchase-card" data-status={card.state} data-purchase-id={card.purchaseId} key={card.id}>
        ...
        <div className={`purchase-confirmation-card ${card.state}`}>
          ...
          {card.state === "ready" && <><p>{mode === "live" ? `실제 Base Sepolia ${symbol} 전송이 한 번 발생할 수 있으며 Provider 응답은 Mock입니다.` : mode === "mock" ? "Mock 결제에서는 토큰을 전송하지 않습니다." : "결제 모드를 확인한 뒤에만 실행할 수 있습니다."}</p><p>이 카드의 고정 요청·예산·우선순위로 한 번만 결제를 실행합니다. 최대 8,000 출력 토큰을 반영한 모델별 고정 선결제액입니다.</p>{readinessMessage && <div id={`purchase-readiness-${card.id}`} data-testid="purchase-readiness" className="notice risk">{readinessMessage} {pending?.purchaseId && <Link href={`/purchases/${encodeURIComponent(pending.purchaseId)}`}>저장된 거래 보기 →</Link>}<button type="button" onClick={() => void refreshReadiness()} disabled={busy || pendingChecking}>연결 다시 확인</button></div>}<button className="primary" type="button" data-purchase-confirm onClick={() => void confirm(card)} disabled={confirmBlocked} aria-describedby={readinessMessage ? `purchase-readiness-${card.id}` : undefined}>동의하고 구매 실행</button></>}
          {card.state === "running" && <p>결정·결제·감사 증거를 기록하고 있습니다.</p>}
          {card.state === "error" && <><p className="riskText">{card.error}</p>{card.purchaseId && <Link href={`/purchases/${encodeURIComponent(card.purchaseId)}`}>거래 상세와 증거 보기 →</Link>}{readinessMessage && <div id={`purchase-readiness-${card.id}`} data-testid="purchase-readiness" className="notice risk">{readinessMessage} {pending?.purchaseId && <Link href={`/purchases/${encodeURIComponent(pending.purchaseId)}`}>저장된 거래 보기 →</Link>}<button type="button" onClick={() => void refreshReadiness()} disabled={busy || pendingChecking}>연결 다시 확인</button></div>}<button type="button" onClick={() => { patch(card.id, { state: "ready" }); void confirm({ ...card, state: "ready" }); }} disabled={confirmBlocked} aria-describedby={readinessMessage ? `purchase-readiness-${card.id}` : undefined}>같은 요청 다시 시도</button></>}
