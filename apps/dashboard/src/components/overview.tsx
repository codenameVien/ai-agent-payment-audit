...
export function Overview() {
  ...
  return <main>
    ...
    <section className="twoCol">
      ...
      <article className="panel"><div className="sectionHead"><div><p className="eyebrow">계정과 결제 에이전트</p><h2>자동 연결된 지갑</h2></div><Link href="/login">계정 변경</Link></div><dl className="walletList"><div><dt>로그인 계정</dt><dd>{short(wallet?.owner_address)}</dd></div><div><dt>자동 결제 지갑</dt><dd>{short(wallet?.buyer_wallet_address)}</dd></div><div><dt>PBLC 토큰</dt><dd>{short(wallet?.token)}</dd></div></dl><small>MetaMask는 본인 확인용이고, 구매 에이전트가 별도 지갑으로 정책 범위 안에서 결제합니다.</small></article>
    </section>
