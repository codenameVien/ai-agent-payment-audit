...
Status: Revised and Approved on 2026-09-04
Date: 2026-09-04
Upstream: `aidlc-docs/inception/requirements.md` (Revised 2026-09-04)
...
```mermaid
flowchart LR
  U[사용자 MetaMask] -->|요청·예산·우선순위| RQ[구매 요청 페이지 /request]
  U -->|결과 조회| D[읽기 전용 감사 대시보드 /dashboard]

  subgraph BA[구매 에이전트 논리 경계]
    B[Buyer Agent\n분석·비교·선택·승인·결과 반환]
    BW[Buyer SDK Wrapper\nSeller API · Audit Evidence API · x402 Client · RPC]
    PE[Payment Executor\n키 격리 · x402 서명/실행]
  end

  subgraph SELL[판매 에이전트]
    S[Seller Service\n견적 · 유료 inference]
    SW[Seller SDK Wrapper\nGemini · Nemotron adapters]
  end

  API[FastAPI\nRequest/Audit HTTP API]
  AE[감사 증거 기록 모듈\nAudit Evidence API]
  M[(MongoDB Atlas)]
  ...
  RQ --> API --> B
  B --> BW
  BW -->|벤치마크 조회| BM[Benchmark Source]
  BW -->|견적 API| S
  S --> SW --> MP[Model Providers]
  BW --> AE --> M
  B -->|purchaseId| PE
  PE -->|물리적으로 별도 프로세스/컨테이너| S
  S -->|verify·settle| F[x402 Facilitator]
  F -->|EIP-3009 transferWithAuthorization| T[PBLC V2 ERC-20]
  PE -->|독립 receipt·Transfer 조회| RPC[Base Sepolia RPC]
  D -->|읽기 API| API
  API --> AE
  ...
  class U,RQ,D human;
  class API,B,BW,PE,S,SW,AE service;
  class M,BM,MP,SEC data;
  class F,T,R,RPC chain;
```
...
| Buyer SDK Wrapper | Seller Quote API + Audit Evidence API client + x402 Payment Executor client + Base Sepolia RPC adapter | Buyer Agent가 외부 I/O 구현에 직접 결합되지 않게 함 |
| Seller SDK Wrapper | Gemini API adapter + Nemotron/NVIDIA API adapter | provider/model 교체와 mock/real 모드를 판매 로직에서 분리 |
| MongoDB Logger | 감사 증거 기록 모듈(Audit Evidence API; internal `EvidenceRepository`) | 모든 서비스가 DB에 직접 접근하지 않도록 단일 기록·조회 경계 제공 |
| On-chain Trigger | Payment Executor (internal `CommerceGateway`) | Buyer Agent가 소유하되 물리적으로 격리된 유일한 서명·체인 쓰기 경계 |
...
| `/request` | 구매 요청·예산·우선순위 입력과 Buyer Agent 실행; 대시보드 셸 밖의 독립 화면 | `POST /purchases`, `POST /purchases/{purchaseId}/run` |
| `/`, `/dashboard` | 읽기 전용 감사 개요 | `GET /wallet`, `GET /purchases`, `GET /audit-alerts`, SSE |
| `/experiments` | 레거시 진입점 | `/request`로 서버 리다이렉트 |
...
감사 개요는 구매를 실행하지 않는다. PBLC 잔액, 실제 계정 범위의 거래·결제 해시, 구매 에이전트 판단 근거, 감사 등급, ERC-8004 평판, 경고, RPC/SSE 상태만 조회한다. 구매 입력과 실행은 `/request`에만 존재한다. `/request`는 명시적 테스트넷 결제 확인 뒤 사용자가 선택한 예산과 우선순위를 전달하고, 요청 생성에 성공한 `purchaseId`를 동일 출처의 `localStorage`에 보존해 새 탭이나 실행 실패 후에도 같은 구매를 재개한다. `/experiments`는 데이터 작업 없이 `/request`로 이동한다. 가짜 거래·가짜 경고·비정상 시나리오 실행기는 감사 대시보드에 넣지 않는다.
...
7. `AuditEvidenceClient`: 전체 후보·탈락 이유·점수·승자를 `DECIDED` 이벤트로 고정한다.
8. `PaymentExecutorClient`: 선택된 고정 견적의 결제를 요청하고 결과를 회수한다.
9. 결과를 사용자 요청 흐름에 반환하고 감사 대시보드에서 조회 가능하게 한다.

Buyer Agent가 구매 전체의 논리적 주체다. LLM은 요청 분류와 설명에만 사용하며 후보 탈락, 점수, 예산 통과 여부, 구매 승인, 결제 요청, 최종 감사 등급은 결정적 코드가 소유한다.

#### Buyer SDK Wrapper

현재의 Python/TypeScript adapter 파일을 재사용하되 상위 설계에서는 다음 네 포트를 하나의 Buyer SDK Wrapper로 묶어 설명한다.

- 판매 에이전트 견적 API 호출
- Audit Evidence API 기록·조회 호출
- x402 Payment Executor 클라이언트 호출
- Base Sepolia RPC 영수증·Transfer 조회
...
공통 판매 엔진 아래의 `GeminiAdapter`, `NemotronAdapter`, `MockAdapter`를 Seller SDK Wrapper로 묶는다. 판매 에이전트의 견적·결제 검증 책임과 provider API 요청/응답 변환 책임을 분리한다.
...
### 3.5 Payment Executor (internal code name: Commerce Gateway)

Payment Executor는 Buyer Agent 경계 안의 실행 모듈이지만 개인키 격리를 위해 별도 Node.js 프로세스/컨테이너로 배포한다. FastAPI는 개인키를 보유하지 않으며 금액·수신자를 신뢰 입력으로 보내지 않고 `purchaseId`만 보낸다. Payment Executor가 Audit Evidence API에서 고정된 결정과 견적을 다시 읽어 결제 조건을 만든다.
...
- `transferWithAuthorization(from,to,value,validAfter,validBefore,nonce,v,r,s)`와 `authorizationState(authorizer,nonce)`를 구현한다.
- nonce는 `purchaseId`에서 예측 생성하지 않고 CSPRNG `bytes32`로 한 번 생성해 intent에 고정한다. 재시도는 같은 nonce를 사용하고 새 구매만 새 nonce를 만든다.
- `block.timestamp <= validAfter`는 아직 유효하지 않음, `block.timestamp >= validBefore`는 만료로 거부한다.
...
### 3.6 감사 증거 기록 모듈 (Audit Evidence API)

이 모듈은 에이전트가 아니다. MongoDB 접근은 FastAPI 내부의 `EvidenceRepository` 구현만 수행하고, 사용자 문서·화면·구조도에서는 Audit Evidence API 또는 감사 증거 기록 모듈로 부른다. Seller Service와 Payment Executor는 인증된 내부 API만 사용한다.
...
1. **Deterministic Rules:** 요청 priority와 저장 weights, 후보별 hard-filter 재계산, eligible 최고점과 winner 일치, 설명의 preset/provider/model/score 일치, 예산·정책, 견적/결제 불일치, purchase당 단일 정산, 독립 Transfer, ERC-8004 신원, 해시 체인 손상을 판정한다.
...
sequenceDiagram
  actor U as 사용자
  participant RQ as 구매 요청 /request
  participant B as Buyer Agent
  participant BW as Buyer SDK Wrapper
  participant E as Audit Evidence API
  participant M as MongoDB
  participant S as Seller Service
  participant SW as Seller SDK Wrapper
  participant P as Payment Executor (별도 배포)
  participant F as x402 Facilitator
  participant C as Base Sepolia
  participant D as 감사 대시보드

  U->>RQ: prompt, budget, priority
  RQ->>B: POST /purchases + run
  B->>E: append REQUESTED + encrypted prompt
  E->>M: persist
  B->>BW: quote candidates
  BW->>S: seller quote APIs
  S-->>BW: signed Gemini/Nemotron quotes
  BW-->>B: normalized quotes
  B->>E: append QUOTED
  B->>B: hard filter + weighted score + approve
  B->>E: append DECIDED + evidence hash
  B->>P: execute(purchaseId)
  P->>E: read fixed decision + claim budget
  E->>M: atomic reserve + append intent
  P->>S: inference request
  S-->>P: 402 exact/eip3009 requirements
  P->>P: quote/402/policy/identity verify
  P->>S: retry with signed ERC-3009 payload
  S->>F: verify + settle
  F->>C: transferWithAuthorization
  C-->>F: transaction receipt
  F-->>S: settlement result
  S->>SW: selected provider inference
  SW-->>S: provider response
  S-->>P: response + tx hash
  P->>C: independent Transfer + AuthorizationUsed query
  P->>E: settle payment
  E->>M: finalize reserve + append PAYMENT_SETTLED
  P-->>B: response + verified tx proof
  B->>E: encrypted response + DELIVERED
  B->>B: deterministic + semantic audit
  B->>E: append AUDITED report
  B->>P: submit objective feedback(purchaseId)
  P->>C: ERC-8004 feedbackHash transaction
  P->>E: append REPUTATION_RECORDED
  U->>D: 조회
  D->>E: read-only API through FastAPI
  E-->>D: decision, payment, response, alerts
```
...
Payment Executor는 결제 전 `DecisionAuthorization` EIP-712 서명을 만들어 `decisionHash`와 함께 저장한다. 별도 DecisionLogger/AuditLogger 컨트랙트는 만들지 않는다.
...
판정은 Audit Engine, 제출 서명은 Payment Executor가 담당한다. Payment Executor는 감사 엔진이 보낸 점수를 신뢰하지 않고 Audit Evidence API에서 `AUDITED` 이벤트와 bundle hash를 다시 읽는다.
...
| 중복·동시 결제 | 재시도/두 worker가 동시에 실행 | Mongo claim unique index, atomic budget reservation, purchase에 고정된 transfer method/nonce, 온체인 authorizationState | concurrency + nonce replay tests |
...
- Payment Executor와 Seller Service는 private subnet과 서비스 디스커버리로만 접근한다.
  ...
  buyer-audit-api/
    src/buyer_audit_api/
      ...
      domains/
        ai_inference/
      adapters/
  commerce-gateway/  # internal package; user-facing role is Payment Executor
    src/
      core/
      adapters/
  seller-service/
...
- PBLC V2 정상 서명, 잘못된 서명, 만료, 아직 유효하지 않음, nonce 재사용, 잔액 부족 계약 테스트
- x402 v2 ERC-3009 402/payload exact binding과 method downgrade 거부 테스트
...
1. 기존 PBLC/Permit2 실거래와 MongoDB 성공·미완결 증거 보존 확인
2. PBLC V2 배포 전 주소·지갑·가스·mint/0.1 PBLC 계획 승인
3. 승인 후 x402.org와 가능하면 Coinbase CDP Facilitator를 통한 Base Sepolia `exact + eip3009` `/verify`·`/settle`
4. `AuthorizationUsed`, buyer→seller의 정확한 `Transfer`, nonce 재사용 거부 독립 확인
5. Gemini와 Nemotron 실제 응답 ID·model version·hash
6. MongoDB 저장 레코드 재조회
7. ERC-8004 Identity 및 feedback tx
8. 정상 흐름과 의도적 위반 흐름의 대시보드 캡처
...
| FastAPI 단일 Buyer/Audit HTTP 서비스 | 기존 Python Buyer Agent 로직 재사용, MVP 단순화; 개인키는 별도 Payment Executor에 유지 | 초기부터 독립 audit microservice |
...
| US-02 | `/request`, Buyer Agent, Audit Evidence API |
| US-03 | Buyer SDK Wrapper, Benchmark Provider, Seller Service |
| US-04 | HardFilter, WeightedScorer, Explanation Adapter |
| US-05 | Buyer Agent, Payment Executor, walletPolicies, Seller x402 middleware, PBLC V2 |
| US-06 | Seller Service, Sensitive Payload Store |
| US-07 | Audit Engine |
| US-08 | Audit Evidence API/internal `EvidenceRepository`, Payment Executor signer, ERC-8004 adapter |
| US-09 | Read-only audit dashboard, Query API, SSE |
...
- [x402 v2 specification](https://github.com/x402-foundation/x402/blob/main/specs/x402-specification-v2.md) — v2 envelope, CAIP-2, `assetTransferMethod`
- [x402 exact EVM scheme](https://github.com/x402-foundation/x402/blob/main/specs/schemes/exact/scheme_exact_evm.md) — EIP-3009 wire payload, default method, verify/settle semantics
- [Coinbase CDP Facilitator](https://docs.cdp.coinbase.com/x402/seller/facilitator) — Base Sepolia x402 v2 exact와 EIP-3009/Permit2 ERC-20 지원
- [ERC-3009](https://eips.ethereum.org/EIPS/eip-3009) — authorizationState, validAfter/validBefore, random bytes32 nonce, AuthorizationUsed
