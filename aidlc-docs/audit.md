...
## 2026-09-12 — 챗봇 UI 및 반복 구매 사용자 교정

- 사용자가 기존 UI를 챗봇 스타일로 보지 않았으며 “작성 중인 메시지를 먼저 보내거나 지워 주세요” 경고가 반복 구매를 방해한다고 지적했다.
- 폼 속 대화 초안을 단일 채팅 + 요청별 실행 확인 카드로 교정한다. 메시지 보내기는 결제하지 않으며 카드의 명시 실행 동의만 구매를 호출한다. 성공/실패를 대화 안에 남기고 새 요청과 동일 ID 재시도를 구분한다.
- [수정 설계](../docs/DECISION_OBSERVER_PLAN.md)의 채팅 UX 교정 범위만 적용한다. 기존 기록, 지갑·키, 결제/Anchor 백엔드, AWS 및 외부 실행 승인은 변경하지 않는다.

## 2026-09-12 — 실행 버튼 무응답 후속 교정

- 사용자가 확인 카드의 `동의하고 구매 실행` 버튼이 눌리지 않는 화면을 제시했다. 로컬 점검에서 3100 포트 연결 거부/리스너 없음을 확인했다. 코드에는 최초 health 실패의 재시도가 없고, 버튼 근처에 차단 이유도 없었다.
- 기존 Mock preview 명령으로 서버를 복구하고, [연결 복구 설계](../docs/DECISION_OBSERVER_PLAN.md)에 따라 상태 조회 실패→재확인→명시 실행의 복구 경로를 수정한다. 미확정/404 거래 삭제, 실제 송금, 배포 및 별도 보안 강화는 범위에 넣지 않는다.
